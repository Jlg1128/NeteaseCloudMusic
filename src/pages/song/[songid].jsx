...
class Songinfo extends Component {
	...
	componentDidMount() {
		const currenuserid = Cookies.get('uid');
		// 判断用户状态
	...
	render() {
		...
		const addcommentModule = comments.length > 0
			? <AddComment
				dispatch={dispatch}
				handelSubmitComment={this.handelSubmitComment}
				musicid={musicid}
				userid={userinfo.userId}
				logstatus={userlogstatus}
				avatar={userinfo.avatarUrl}
				username={userinfo.nickname} /> : '';
		const commentsModule = comments.length > 0
			? comments.map((item, index) => (<Comment
				...
				handelSubmitComment={this.handelSubmitComment}
				logstatus={userlogstatus}
				comment={item} />)) : <div key={null}>Loding.....</div>;
		return (
  <div>
        ...
        <div className={`${cssobj.main_left} `}>
          ...
          <div className={`${cssobj.wrap} ${cssobj.clearfix}`}>
            ...
            <div className={`${cssobj.song_main}  ${cssobj.clearfix}`}>
              <div className={cssobj.songname}>
                <div>
                  <span>
                    <img style={{ width: 35, height: 35 }} src="/static/tag.png" alt="" />
                    单曲
                  </span>
                  <h1 className={cssobj.h1}>{comments.length > 0 ? musicInfo.songname : '加载中'}</h1>
                </div>
                <div>
                  &nbsp;&nbsp;歌手:
              ...
              <div className={`${cssobj.lyrics} `}>
                <div className={cssobj.lyrics_singer}>
                  {lrc.slice(0, 5).map((item, index) => <p key={index}>{item}</p>)}
                </div>
                <div className={cssobj.lyrics_showed}>
                  {lrc.slice(5, 12).map((item, index) => <p key={index}>{item}</p>)}
                </div>
          ...
          <div className={`${cssobj.comment_wrap} ${cssobj.clearfix}`}>
            <div>
              ...
              <i style={{ paddingBottom: 3 }}>
                共
                {commentsLength}
                条评论
              </i>
            </div>
            <div className={cssobj.redline} />
            {addcommentModule}
              ...
              {similarMusic.map((item, index) => (
                <li key={item.name}>
                  ...
                  <span className={cssobj.li_singername}>
                    <PlayCircleOutlined
													onClick={() => this.clickMusic(item.id)}
													className={cssobj.play}
													style={{ fontSize: 20 }} />
...
function mapStateToProps(state) {
	return {
		...
		songabout: state.songabout,
		userinfo: state.userinfo.userinfo,
		userlogstatus: state.userinfo.userlogstatus,
	};
}
