...
class TopItem extends Component {
  ...
  render() {
    ...
    const cloudTese = ranklist ? (
      ranklist.map((item, index) => (
        <li
          onClick={() => {
            this.setState({ clickIndex: index });
          }}
          style={{ backgroundColor: clickIndex === index ? '#ccc' : '#f9f9f9' }}
          key={index}
        >
          <Link to={`/discover/toplist/${item.id}`}>
            {' '}
            <div>
    ...
    return (
      <div className={`${cssobj.Toplist_bgc} ${cssobj.clearfix}`}>
        <div className={cssobj.Toplist_wrap}>
          <div className={cssobj.Toplist_item}>
            <div className={cssobj.tese}>
              ...
              <ul className={cssobj.toplist_ul}>{cloudTese}</ul>
            </div>
          </div>
        </div>
      </div>
