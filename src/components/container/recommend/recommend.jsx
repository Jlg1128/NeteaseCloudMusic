    ...
    render() {
        ...
        const dailyRecommend = () => {
            if (dailyRecommendMusic.length > 0) {
                return dailyRecommendMusic.map((item, index) => (
                  <li onClick={() => history.push(`/song/${item.id}`)} key={index} className={cssobj.musci_link}>
                    <a key={index} href=""><div><img src={`${item.picUrl}?param=140y140`} alt="" /></div></a>
            ...
            return (
              <li>
                <img
                src={`${url}?param=140y140`}
                onClick={() => { history.push('/song/1460930236'); }}
                alt="" />
              </li>);
        };
        // 新碟上架
        const newAblum = () => {
            if (NewestAblum.length > 0) {
                return (NewestAblum.map((item, index) => (
                  <li onClick={() => history.push(`/ablum/${item.id}`)} key={index}>
                    <a href=""><div><img src={`${item.blurPicUrl}?param=140y140`} alt="" /></div></a>
                    <a href=""><h3>{item.name}</h3></a>
            ...
            return (<li>
              <img
                src={`${url}?param=140y140`}
                onClick={() => { history.push('/ablum/1460930236'); }}
                alt="" />
            </li>);
        };
        // 入驻歌手
        const joinedsingerfun = () => {
            if (dailyRecommendMusic.length > 0) {
                return HotJoinedSinger.map((item, index) => (
                  <div
                  onClick={() => history.push(`/user/home/${item.accountId}`)}
                  key={index}
                  className={cssobj.zhubolist}>
                    <div><img src={`${item.picUrl}?param=30y30`} alt="" /></div>
                    <a href="">
                      <h3>
                        {item.name}
                        <SketchOutlined style={{ color: 'red' }} />
                      </h3>
                      {' '}
                    </a>
                  </div>
              ...
              <img
                src={`${url}`}
                onClick={() => { history.push('/user/home/1460930236'); }}
                alt="" />
            </li>);
        };
        // 入驻主播
        const hotzhubolist = () => {
            if (HotZhubo.length > 0) {
                return HotZhubo.map((item, index) => <div
                onClick={() => history.push(`/user/home/${item.id}`)}
                 key={index}
                 className={cssobj.zhubolist}>
                  <div><img style={{ width: 38, height: 38 }} src={`${item.avatarUrl}?param=40y40"`} alt="" /></div>
                  <a href="">
                    <h3>
                      {item.nickName}
                      <SketchOutlined style={{ color: 'red' }} />
              ...
              <li>
                <img src={`${url}?imageView&quality=89`}
                onClick={() => { history.push('/user/home/1460930236'); }}
                alt="" />
              </li>);
        };
        ...
        const toplisthotlist = () => {
            if (toplisthot.length > 0) {
                return toplisthot.map((item, index) => (
                  <li
                  onClick={() => history.push(`/song/${item.id}`)}
                  key={index}
                  onMouseOut={() => { this.setState({ musiclistindex: -1 }); }}
                   onMouseOver={() => { this.setState({ musiclistindex: index + 20 }); }}>
                    ...
                    <a href="">
                      ...
                      alt="123"
                      title="嘻嘻"
                      style={{ display: (musiclistindex === index + 20 ? 'block' : 'none') }}
                      className={cssobj.play2} />
                    </a>
                    <a href="">
                      <FolderAddOutlined
                      alt="添加"
                      tyle={{ display: (musiclistindex === index + 20 ? 'block' : 'none') }}
                      className={cssobj.add2} />
                    </a>
                  </li>
                    ),
                );
        ...
        const toplistnewestlist = () => {
            if (toplistnewest.length > 0) {
                return toplistnewest.map((item, index) => (
                  <li
                  onClick={() => history.push(`/song/${item.id}`)}
                  key={index}
                  onMouseOut={() => { this.setState({ musiclistindex1: -1 }); }}
                  onMouseOver={() => { this.setState({ musiclistindex1: index + 20 }); }}>
                    <i className={cssobj.i} style={{ color: (index > 2) ? '' : 'red' }}>{index + 1}</i>
                    <a href="">{item.name}</a>
                    <a href="">
                      <PlayCircleOutlined
                      alt="123"
                      title="嘻嘻"
                      style={{ display: (musiclistindex1 === index + 20 ? 'block' : 'none') }}
                      className={cssobj.play2} />
                    </a>
                    <a href="">
                      <FolderAddOutlined
            ...
            return (<li onClick={() => { history.push('/song/1460930236'); }} />);
        };
        // 原创榜
        const toplisthotoriginlist = () => {
            if (toplisthotorigin.length > 0) {
                return toplisthotorigin.map((item, index) => (
                  <li
                  onClick={() => history.push(`/song/${item.id}`)}
                  key={index}
                  onMouseOut={() => { this.setState({ musiclistindex2: -1 }); }}
                   onMouseOver={() => { this.setState({ musiclistindex2: index + 20 }); }}>
                    ...
                    <a href="">
                      ...
                      alt="123"
                      title="嘻嘻"
                      style={{ display: (musiclistindex2 === index + 20 ? 'block' : 'none') }}
                      className={cssobj.play2} />
                    </a>
                    <a href="">
                      <FolderAddOutlined
            ...
            if (userloginfo.userlogstatus) {
                return (<div>
                  ...
                  <ul className={cssobj.dongtai}>
                    <li style={{ cursor: 'point' }} onClick={() => history.push(`/user/event?id=${userinfo.userId}`)}>
                      <span>
                        <h5>{userinfo.authority}</h5>
                        <h4>
                          <a href="">动态</a>
                          {' '}
                        </h4>
                    ...
                    <li style={{ cursor: 'point' }} onClick={() => history.push(`/user/follows?id=${userinfo.userId}`)}>
                      <span>
                        <h5>{userinfo.follows}</h5>
                        <h4>
                          <a href="">关注</a>
                          {' '}
                        </h4>
                    ...
                    <li style={{ cursor: 'point' }} onClick={() => history.push(`/user/fans?id=${userinfo.userId}`)}>
                      <span>
                        <h5>{userinfo.followeds}</h5>
                        <h4>
                          <a href="">粉丝</a>
                          {' '}
                        </h4>
            ...
            <div className={cssobj.reco_musiclist}>
              ...
              <div className={`${cssobj.musiclist_main} ${cssobj.clearfix}`}>
                ...
                <div className={cssobj.musci_link_wrap2}>
                  {/* <a href="">
                    <LeftOutlined className="arrow lf" />
                  </a>
                  <a href="">
                    <RightOutlined className="arrow ri" />
                  </a> */}
                  <ul>
                    {newAblum()}
                  </ul>
